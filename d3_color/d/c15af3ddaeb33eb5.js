// https://observablehq.com/@matheusbarrio/taxa-de-desemprego-nos-eua-em-agosto-de-2016@84
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","topojson","us","rateById"], function(d3,DOM,topojson,us,rateById)
{
  ...
  let dgreens = d3.scaleQuantize().domain([1,10]).range(d3.schemeGreens[9])
  const countie = svg.append("g")
      .attr("class", "counties")
    .selectAll("path")
      .data(topojson.feature(us, us.objects.counties).features)
    .enter().append("path")
      .attr("fill", d => dgreens(rateById.get(d.id)))
      .attr("d", path)
  countie.append("title").text(function (d) {
