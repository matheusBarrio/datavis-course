// https://observablehq.com/@matheusbarrio/d3-com-crossfilter-e-dc-js@271
...
require("d3")
  ...
  main.variable(observer("googleByDayVariation")).define("googleByDayVariation", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => ((d.google - dateDim.bottom(1)[0].google) / dateDim.bottom(1)[0].google) * 100)
)});
  main.variable(observer("fbByDayVariation")).define("fbByDayVariation", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => ((d.facebook - dateDim.bottom(1)[0].facebook) / dateDim.bottom(1)[0].facebook) * 100)
)});
  main.variable(observer("acoes")).define("acoes", ["md","container","dc","d3","dateDim","googleByDayVariation","fbByDayVariation"], function(md,container,dc,d3,dateDim,googleByDayVariation,fbByDayVariation)
{
  ...
  compositeChart.width(800)
              ...
              .compose([
                  dc.lineChart(compositeChart)
                    .group(googleByDayVariation, 'Google')
                    .ordinalColors(['steelblue']),
                  dc.lineChart(compositeChart)
                    .group(fbByDayVariation, 'Facebook')
                    .ordinalColors(['darkorange'])])
  dc.renderAll()
  return view      
}
...
d3.json('https://raw.githubusercontent.com/emanueles/datavis-course/master/assets/files/observable/movies.json').then(function(dataset){
...
facts2.dimension( d => d.Year)
  ...
  main.variable(observer("mdollaresAnoGrupo")).define("mdollaresAnoGrupo", ["dateDim2"], function(dateDim2){return(
dateDim2.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("by_year")).define("by_year", ["md","container","dc","d3","dateDim2","mdollaresAnoGrupo"], function(md,container,dc,d3,dateDim2,mdollaresAnoGrupo)
{
  ...
  barChart
    .width(800).height(400).dimension(dateDim2)
    .margins({top: 30, right: 50, bottom: 25, left: 40})
    .x(xScale)
    .renderHorizontalGridLines(true)
    .legend(dc.legend().x(650).y(10).itemHeight(13).gap(5))
    .brushOn(false)
    .group(mdollaresAnoGrupo, "Bilheteria(Mi)")
  ...
  main.variable(observer("generodim")).define("generodim", ["facts2"], function(facts2){return(
facts2.dimension( d => d.Genre)
)});
  main.variable(observer("mDollaresGeneroGrupo")).define("mDollaresGeneroGrupo", ["generodim"], function(generodim){return(
generodim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("by_genre")).define("by_genre", ["md","container","dc","d3","generodim","mDollaresGeneroGrupo"], function(md,container,dc,d3,generodim,mDollaresGeneroGrupo)
{
  ...
  let xScale = d3.scaleOrdinal()
                  .domain(generodim)
  barChart
    .width(800).height(400).dimension(generodim)
    .margins({top: 30, right: 50, bottom: 25, left: 40})
    .x(xScale)
    .ordering(function(x) { return -x.value})
    .xUnits(dc.units.ordinal)
    .renderHorizontalGridLines(true)
    .legend(dc.legend().x(650).y(10).itemHeight(13).gap(5))
    .brushOn(false)
    .group(mDollaresGeneroGrupo, "Bilheteria(Mi)")
  barChart.gap(25)
  barChart.render()
  return view      
}
